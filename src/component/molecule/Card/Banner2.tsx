...
type banner2Prop = {
  data: {id: number; image: any; route?: string; onPress?: any}[];
  delay?: number;
};
  ...
  const renderItem = ({item}: any) =>
    data.length > 1 ? (
      <TouchableOpacity
        ...
        onPress={() => console.log(`navigates to ${item.route}`)}>
        {/* <Image
      ...
      renderDots={(activeIndex: number) =>
        data.map((_: any, i: number) => (
          <View style={{alignItems: 'center'}} key={i}>
            {i == activeIndex ? (
              <DotIcon width={8} height={8} style={{marginHorizontal: 5}} />
            ) : (
              <DashIcon
                scaleX={1.2}
                scaleY={1.2}
      ...
      <View style={styles.container}>
        <Carousel
          ...
          getItemLayout={(data: any, index: number) => ({
            length: moderateScale(parseInt(cardsBannersWidth)),
            offset: moderateScale(parseInt(cardsBannersWidth)) * index,
            index,
          })}
          horizontal={true}
          loop={true}

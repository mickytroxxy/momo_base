...
export interface Props extends Omit<TextInputProps, 'secureTextEntry'> {
  ...
  rightComponent?: keyof typeof SvgIcons | JSX.Element;
...
const FloatingLabelInput: React.ForwardRefRenderFunction<InputRef, Props> = (
  ...
  function handleFocus() {
    setIsFocused(true);
    name && logInputFocusEvent(`inputfocus_${name.replace(/\s/g, '')}`);
  }

  function handleBlur() {
    // if (value === '') {
    setIsFocused(false);
    name && logInputBlurEvent(`inputblur_${name.replace(/\s/g, '')}`);
    // }
  }
  ...
  let input: TextStyle =
    inputStyles !== undefined
      ...
      : {
          ...(styles.input as {}),
          minHeight: medium ? gpmsh('56') : gsh(56),
              ...
              <View style={{flex: 1, flexDirection: 'row'}}>
                ...
                <View
                  style={[
                    style,
                    {
                      ...
                      paddingVertical: 1,
                      position: 'absolute',
                      top: isFocusedState || value !== '' ? -11 : '32%',
                ...
                <Box style={toggleButton} alignSelf={'center'}>
                  {rightComponent && !kurrency && (
                    <Icon
                      name={rightComponent}
                      onPress={() => {
                        name && logInputIconPressEvent(name);
                      }}
                      color={
                        error
                          ? colors.red100
                ...
                {isPassword && (
                  <TouchableOpacity
                    ...
                    <Icon
                      name={secureText ? 'EyeIcon' : 'EyeslashIcon'}
                      stroke={
                        error
                          ? colors.red100
                          : isFocusedState
      ...
      {kurrency &&
        (error ? (
          ...
          <Text
            ...
            textAlign={'right'}
            color={isFocusedState ? 'momoBlue' : 'black'}>
